...
def parse_args():
    ...
    parser.add_argument('--planetary_computer_subscription_key', type=str, default='', help='Planetary Computer subscription key.')
    ...
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size for inference (not recommended to change)')
    parser.add_argument('--num_workers', type=int, default=4, help='Number of worker processes for data loading.')
    parser.add_argument('--input_size', type=int, default=128, help='Size of input patches (the model expects 128)')
    parser.add_argument('--run_vv_only', action='store_true', default=False, help='Run flood detection using only VV polarization.')
...
def main():
    ...
    # Setup data module
    dm = FloodDataModule(args.batch_size, args.num_workers, args.region, [args.start_date, args.end_date], 1./args.scale_factor, args.run_vv_only)
    dm.setup()
    # Run inference
    with torch.no_grad():
        for batch in dm.test_dataloader():
            ...
            for i, (filename, year, month, day) in enumerate(zip(filenames, years, months, days)):
                ...
                try:
                    ...
                    predimg, _ = reconstruct_image_from_patches(predicted_all, padded_shapes[i], (args.input_size, args.input_size), args.input_size)
