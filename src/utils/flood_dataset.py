    ...
    def read_image_data(self, url, max_retries=2, delay=0.1):
        ...
        while retries < max_retries:
            try:
                with rasterio.open(url) as dataset:
                    if self.scale_factor == 1:
                        ...
                        data_transform = Affine(
                            dataset.transform.a / self.scale_factor,
                            dataset.transform.b,
                            dataset.transform.c,
                            dataset.transform.d,
                            dataset.transform.e / self.scale_factor,
                            dataset.transform.f,
                        )
                ...
                retries += 1
                print(
                    f"Attempt {retries} failed for url {url}. Error: {str(e)}. Retrying after {delay * retries} seconds."
                )
    ...
    def __getitem__(self, index):
        ...
        if self.run_vv_only:
            ...
            if vv_pre.shape != vh_pre.shape:
                vh_pre = self.reproject_image(
                    vh_pre, vh_ref_crs, vh_ref_transform, vv_ref_crs, vv_ref_transform, vv_pre.shape
                )
            vh_post = self.reproject_image(vh_post, vh_crs, vh_transform, vv_ref_crs, vv_ref_transform, vv_pre.shape)
        # Calculate change
        vv_change = (
            (vv_post < self.vv_threshold) & (vv_pre > self.vv_threshold) & ((vv_pre - vv_post) > self.delta_amplitude)
        ).astype(int)
        if self.run_vv_only:
            vh_change = vv_change
        ...
        return {
            ...
            "padded_shape": rgb_image.shape[:2],
            "crs": vv_ref_crs,
            "transform": vv_ref_transform,
            "ignore_flag": 0,
        }
